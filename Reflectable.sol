...
abstract contract Ownable is Context {
    ...
    function lowerOwnerAccessBy1() onlyOwner external { // Lowers access level for the owner. Once lowered it cannot be increased.
        if (sudo == 0) { return; } 
        emit AccessChanged(--sudo, block.timestamp);
    }

    function _revokeOwnerPrivileges() private { 
        if (sudo == 0) { return; } 
        sudo = 0; 
        emit AccessChanged(0, block.timestamp);
    }

    function revokeOwnerPrivileges() external onlyOwner() { _revokeOwnerPrivileges(); }
...
/** @author Jasper Wolf (https://www.github.com/Ashraile)  Requires Solidity 0.8+
    @dev Next-gen ERC20 (BEP20) token generation framework. @custom:ref https://github.com/bnb-chain/BEPs/blob/master/BEPs/BEP20.md
*/
